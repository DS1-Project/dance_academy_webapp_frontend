...
import { computeLandingCatalogStats } from "@/lib/landingStats";

export function HeroSection() {
  const { items, loading } = useCatalogChoreographies();
  const stats = useMemo(() => computeLandingCatalogStats(items), [items]);
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden pt-20">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute bottom-20 -left-32 w-80 h-80 rounded-full bg-secondary/10 blur-3xl animate-float"
      ...
      <div className="container relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="mb-6 animate-reveal-up" style={{ animationDelay: "100ms" }}>
            Domina el ritmo. <span className="text-gradient">Conquista</span> el escenario.
          </h1>

          <p
            className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 animate-reveal-up"
            style={{ animationDelay: "200ms", textWrap: "pretty" }}
          >
            Aprende coreografías completas de salsa, bachata, hip-hop y más con los mejores
            profesores. Videos profesionales, a tu ritmo, desde cualquier lugar.
          </p>
          ...
          <div
            className="flex flex-wrap items-center justify-center gap-8 md:gap-16 animate-reveal-up"
            ...
            {loading ? (
              ...
              <>
                <div className="text-center">
                  <p className="text-3xl md:text-4xl font-display font-extrabold text-gradient">
                    {stats.choreographyCount}
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">Coreografías</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl md:text-4xl font-display font-extrabold">{stats.genreCount}</p>
                  <p className="text-sm text-muted-foreground mt-1">Géneros</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl md:text-4xl font-display font-extrabold">{stats.teacherCount}</p>
